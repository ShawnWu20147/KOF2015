...
public class FormationPanel extends JPanel {
	 ...
	 */
	public void passingCandidates( final FighterInfo[] candidates )
	{
		...
		for( final MemberDropdownPanel tile : tiles )
		{
			...
			tile.addImageClickListener(new ActionListener() {
				@Override
				public void actionPerformed(ActionEvent e) {
					updateInfo(tile);
				}
	 ...
	 * 因为还没有数据，所以数据的初始化要滞后。
	 */
	public FormationPanel(ClientOrigin cog,String oppoName)
	{
	...
	private void updateInfo( MemberDropdownPanel tile )
	{
		for( int i = 0; i < tiles.length; i++ )
		{
			if( tiles[ i ] == tile )
				infoLabel.setText(candidates[ tiles[ i ].getSelectedIndex() ].toString());
		}
	}
